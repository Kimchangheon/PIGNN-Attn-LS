...
def case_generation_ieee14_pandapower(
    ...
    IEEE-14 preset using pandapower's internal ppci for BOTH:
      - Y_matrix: from ppc_int["Ybus"] (per-unit) converted to SI with per-bus Vbase
      - edge metadata in SI for branch-wise reconstruction (same convention as stamped)

    UNITS (returned):
      - Y_matrix                 : SI [S]
      - Y_Lines                  : SI [S]  (series admittance for *lines* only)
      - Y_C_Lines                : SI [S]  (per-end line charging susceptance for *lines* only)
      - Trafo_y_series           : SI [S]  (series admittance, HV-referred, for *trafos* only)
      - Trafo_b_total            : SI [S]  (magnetizing susceptance, HV-referred, three-phase)
      - Y_shunt_bus              : SI [S]  (bus shunts GS+jBS)
      - s_multi                  : W + j var
      - u_start                  : V
      - vn_kv                    : kV
      - Trafo_tap_on_i, hv_is_i  : int8 (0/1)
      - Trafo_n                  : float64 (>=1, Vh/Vl)
    ...
    Returns:
      (gridtype, bus_typ, s_multi, u_start, Y_matrix, is_connected,
       ...
       Trafo_tap_on_i, Trafo_hv_is_i, Trafo_n)
    """
    import numpy as np
    import pandas as pd
    import pandapower as pp
    ...
    # ---- Bus shunt vector (SI) ----
    # if you already have get_Y_shunt_bus_SI(ppc_int) you can call it;
    # this is the explicit version:
    Ysh_pu = (bus_ppc[:, GS] + 1j * bus_ppc[:, BS]) / baseMVA
    Y_shunt_bus = Ysh_pu * (S_base / (Vbase * Vbase))  # SI [S]
    ...
    Y_Lines = np.zeros(C, dtype=np.complex128)      # SI [S], lines only
    Y_C_Lines = np.zeros(C, dtype=np.float64)       # SI [S], per-end, lines only

    Is_trafo = np.zeros(C, dtype=np.int8)
    Trafo_tau = np.ones(C, dtype=np.float64)
    Trafo_shift_deg = np.zeros(C, dtype=np.float64)
    Trafo_y_series = np.zeros(C, dtype=np.complex128)  # SI [S], HV-referred
    Trafo_b_total = np.zeros(C, dtype=np.float64)      # SI [S], HV magnetizing

    Trafo_tap_on_i = np.zeros(C, dtype=np.int8)
    Trafo_hv_is_i = np.zeros(C, dtype=np.int8)
    Trafo_n = np.ones(C, dtype=np.float64)

    # ---- Fill line/trafo series + line charging from branch_ppc (pu -> SI) ----
    for row in branch_ppc:
        ...
        # upper-tri mapping + tap orientation from "from-bus" side
        if fb < tb:
            i, j = fb, tb
            tap_on_i = 1
        else:
        ...
        is_tr = (abs(tap - 1.0) > 1e-12) or (abs(shift) > 1e-12)

        if not is_tr:
            # line: use local base V_line^2
            V_line = Vi
            scale = S_base / (V_line ** 2)
            Y_Lines[k] = y_pu * scale
            Y_C_Lines[k] = (0.5 * b) * scale
        else:
            # transformer-like: HV-referred series
            Is_trafo[k] = 1
            Trafo_tau[k] = tap
    ...
    # ---- Magnetizing susceptance from net.trafo (SI, HV side) ----
    if len(net.trafo):
    ...
    return (
        ...
        Trafo_tap_on_i.astype(np.int8),
        Trafo_hv_is_i.astype(np.int8),
        Trafo_n.astype(np.float64),
    )
def case_generation_ieee14_pandapower_stamped(
    ...
    # ---- Get ppci internals (no need for convergence) ----
    try:
        pp.runpp(
            net,
            init="flat",
    ...
    # ---- Y_matrix: stamp in pu, convert to SI ----
    Y_pu_stamp = build_Y_stamped_from_ppc(ppc_int)          # per-unit (N,N)
    Y_matrix = per_unit_to_SI(Y_pu_stamp, ppc_int)          # SI (N,N) Siemens

    # ---- Bus types ----
    bus_typ = np.full(N, 3, dtype=int)  # PQ
    if len(net.ext_grid):
    ...
    # ---- Initial voltages u_start in SI ----
    u_start = Vbase.astype(np.complex128)

    if rand_u_start:
        mag = Vbase.copy()

        # PV buses: respect vm_pu
        if len(net.gen):
            for _, row in net.gen.iterrows():
                b = int(row.bus)
                vm = float(row.vm_pu) if pd.notna(row.vm_pu) else 1.0
                mag[b] = vm * Vbase[b]
    ...
    Trafo_shift_deg = np.zeros(C, dtype=np.float64)
    Trafo_y_series  = np.zeros(C, dtype=np.complex128)  # SI [S], HV-referred
    Trafo_b_total   = np.zeros(C, dtype=np.float64)     # SI [S], HV-referred magnetizing
    Trafo_tap_on_i  = np.zeros(C, dtype=np.int8)        # 1 if tap on iu side
    Trafo_hv_is_i   = np.zeros(C, dtype=np.int8)        # 1 if HV base on iu side
    Trafo_n         = np.ones(C, dtype=np.float64)      # Vh/Vl (>=1)

    # ---- Fill line/trafo series + charging from branch_ppc (pu → SI) ----
    for row in branch_ppc:
        fb = int(row[F_BUS])      # already 0-based internal
        tb = int(row[T_BUS])
        r  = float(row[BR_R])
        x  = float(row[BR_X])
        b  = float(row[BR_B])     # total line charging susceptance in pu
        tap = float(row[TAP])
        shift = float(row[SHIFT])
        status = int(row[BR_STATUS])
        ...
        # map to upper-tri (i<j) and record tap orientation (from-bus side)
        if fb < tb:
            i, j = fb, tb
            tap_on_i = 1
        else:
        ...
        if not is_tr:
            # ----- line: convert using V_line^2 (IEEE14: Vi==Vj for lines) -----
            V_line = Vi
            scale = S_base / (V_line ** 2)   # pu → SI scale for this line
            Y_Lines[k]   = y_pu * scale
            Y_C_Lines[k] = (0.5 * b) * scale  # store per-end in SI
        else:
            # ----- transformer-like branch: store HV-referred series admittance in SI -----
            Is_trafo[k] = 1
            Trafo_tau[k] = tap
            Trafo_shift_deg[k] = shift
            Trafo_tap_on_i[k] = tap_on_i

            scale_tr = S_base / (Vh ** 2)     # HV-referred pu → SI
            Trafo_y_series[k] = y_pu * scale_tr
            # NOTE: we do NOT use BR_B for trafos as magnetizing here

    # ---- Magnetizing susceptance from net.trafo nameplate (SI, HV side) ----
    if len(net.trafo):
        for _, tr in net.trafo.iterrows():
            hv = int(tr.hv_bus)
            lv = int(tr.lv_bus)
            ...
            if pfe_kw == 0.0 and i0_pct == 0.0:
                continue  # IEEE14 typically ends here (keeps zeros)
            ...
            # three-phase branch susceptance (HV-referred)
            B_phase = (np.sqrt(3.0) * I_m) / V_ll  # Siemens
            Trafo_b_total[k] = B_phase

    # ---- Bus shunts from GS/BS (SI) ----
    # MATPOWER convention: Ysh_pu = (GS+jBS)/baseMVA, then to SI with S_base/V^2
    GS_pu = bus_ppc[:, GS]
    BS_pu = bus_ppc[:, BS]
