...
def case_generation_ieee14_pandapower(
    ...
    IEEE-14 preset using pandapower's internal ppci for BOTH:
      ...
      - edge metadata in SI for branch-wise reconstruction
    ...
    UNITS (returned):
      ...
      - Trafo_b_total            : SI [S]  (TOTAL shunt susceptance for trafos, HV-referred, derived from BR_B)
                                   (MATPOWER convention: j*BR_B/2 added to each end in pu)
    ...
    Trafo_b_total = np.zeros(C, dtype=np.float64)      # SI [S], TOTAL shunt (from BR_B), HV-referred

    Trafo_tap_on_i = np.zeros(C, dtype=np.int8)
    Trafo_hv_is_i = np.zeros(C, dtype=np.int8)
    Trafo_n = np.ones(C, dtype=np.float64)

    # ---- Fill line/trafo series + shunts from branch_ppc (pu -> SI) ----
    for row in branch_ppc:
        fb = int(row[F_BUS])
        tb = int(row[T_BUS])
        r = float(row[BR_R])
        x = float(row[BR_X])
        b = float(row[BR_B])     # MATPOWER: total shunt susceptance in pu (line charging or compiled trafo shunt)
        tap = float(row[TAP])
        ...
        # Robust "transformer-like" detection: tap/shift OR different base voltage
        ...
        if not is_tr:
            ...
            Y_C_Lines[k] = (0.5 * b) * scale          # per-end SI
        else:
            # ---- transformer-like: HV-referred series + TOTAL shunt from BR_B ----
            Is_trafo[k] = 1
            Trafo_tau[k] = tap
            ...
            Trafo_b_total[k] = b * scale_tr           # TOTAL SI, HV-referred (j*b/2 each end in pu)
...
def case_generation_ieee14_pandapower_stamped(
    ...
    IEEE-14 preset using pandapower's internal ppci for BOTH:
      ...
      - metadata: derived from ppci (bus/branch)
    ...
    # (These helpers are assumed to exist in your codebase exactly as before.)
    ...
    for row in branch_ppc:
        ...
        r  = float(row[BR_R])
        x  = float(row[BR_X])
        b  = float(row[BR_B])     # MATPOWER: total shunt susceptance in pu
        tap = float(row[TAP])
        shift = float(row[SHIFT])
        status = int(row[BR_STATUS])
        ...
        if not is_tr:
            ...
            scale_tr = S_base / (Vh ** 2)
            Trafo_y_series[k] = y_pu * scale_tr
            Trafo_b_total[k]  = b * scale_tr       # TOTAL SI, HV-referred
